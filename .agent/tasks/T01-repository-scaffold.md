...
task rather than a side effect of the first real one. **`tools/` declares no application
dependencies**, so a Tree Author runs `npm ci --workspace tools` and never installs
Svelte — this is how N6 and F40 are satisfied concretely rather than aspirationally.
...
- Root `package.json`: workspaces, scripts only, no runtime dependencies.
- `tools/` workspace — deps limited to `yaml`, `ajv`, `commander`. The `lst` bin entry
  wired but implementing nothing.
...
- ESLint with `no-restricted-imports` scaffolded but rules empty — §14.1's forbidden
  edges get their real entries when the modules they govern exist.
- Node version pinned in `engines` and in `.nvmrc`.
- `CLAUDE.md` at the repository root.
- `.gitignore` additions: `app/build/`, `app/.svelte-kit/`, `node_modules/`, and
  `app/static/content/` — compiled JSON is a build artifact and §4.3 says it is not
  committed.
...
content/.gitkeep  schema/.gitkeep  docs/  .github/workflows/.gitkeep
```
...
- [ ] `npm ci` at the root installs both workspaces without error.
- [ ] `npm ci --workspace tools` succeeds and installs **no** Svelte, SvelteKit, or Vite
      package — verify with `npm ls --workspace tools svelte` returning empty.
- [ ] `npm test` runs Vitest in both workspaces and passes.
- [ ] `npm run typecheck` passes; `tsconfig.base.json` sets `strict: true` and neither
      workspace overrides it to false.
- [ ] `npm run build --workspace app` produces `app/build/` containing `.nojekyll`.
- [ ] `npx svelte-check` runs in `app/` and reports zero errors.
- [ ] `npx lst --help` prints usage and exits 0.
- [ ] `node --version` satisfies the `engines` field, and `engines` requires >= 20.
- [ ] The §4.2 tree exists in full; no directory from the spec is missing.
- [ ] `app/static/content/` is gitignored.
- [ ] `CLAUDE.md` states the Svelte 4 → 5 rune mapping explicitly and links
      `svelte.dev/docs/svelte/llms.txt`.
- [ ] `eslint.config.js` loads and runs clean.

## Verification

```bash
npm ci && npm test && npm run typecheck && npm run build
npm ls --workspace tools svelte    # must find nothing
```

Passing looks like four green commands and an empty dependency query.
...
- **Node 18 is too old** for Vite 8. Pin Node 20 LTS or newer.
