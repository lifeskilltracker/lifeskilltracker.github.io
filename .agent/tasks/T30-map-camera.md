...
- [ ] `/` and `/d/<domainId>` are both prerendered; the built output contains an HTML file
      for every domain.
- [ ] Navigating from `/` into a domain animates the camera and does not remount the map
      surface. Asserted by an identity check on the rendered region paths across the
      transition.
- [ ] Browser Back from level 1 returns to level 0 with the camera flown back, and no
      breadcrumb widget exists anywhere in the markup.
- [ ] Every region renders its plate at `--plate-open` at every score; a domain at fill 0
      and a domain at fill 0.9 differ only by water-line height. Asserted directly.
- [ ] No raw percentage appears in the rendered map (F34). `grep` for `%` in the map's
      accessible names returns nothing.
- [ ] A fogged domain renders hachure, no water line, and the contribute affordance in
      place of its name — driven by the manifest, and unchanged by any user state.
- [ ] The two label world sizes resolve into their specified pixel bands at both levels,
      computed from the real world extent and asserted in a test.
- [ ] Region outlines hold constant screen weight across the transition (1.3 → 0.9 world).
- [ ] Under `prefers-reduced-motion: reduce` the level change is instant and the water line
      does not animate; nothing is lost, per §15.5.
- [ ] Entering a domain announces name, band, breadth and skill count on the polite region.
