...
**Hiding is not a sixth state.** §11.10's "hide it instead" suppresses a node from the drawing and writes nothing, so a hidden milestone has whatever state it always had and gets that state's glyph and border the moment it is revealed. The table above stays five rows; hiding is a filter over which nodes are drawn at all, and it must never borrow `dismissed`'s dotted border or ✕, which would state something about the milestone that the user did not say.
...
**What "hide it instead" is.** It is a **presentation-only suppression held by the open tree session**: the uid stops being drawn — node, edges, and grid position alike — and *nothing is written*. No `MILESTONE` record, no `MilestoneState`, no preference. The milestone stays incomplete, stays in its group's denominator, and stays one click from being completed. That is the whole reason the offer is safe to make at the moment of the intercept: it is the choice that avoids the cap, so an implementation storing any state for it would hand the user the same catastrophe under a gentler word. Because it writes nothing it also does not survive a reload, and the renderer therefore always states the count and offers to reveal the set — a suppression the user cannot find again is indistinguishable from data loss. A durable "hidden" flag would be a §12.2 schema question, and this is deliberately not one.
