...
**Date:** 2026-08-16
**Owner:** Ethan Morchy
**Status:** Phases 0 and 1 implemented; the interface is specified and in build

> **Changes since v1.4.** **D19 closes.** The visual direction is **Survey / Ordnance**, specified in `docs/UI-SPEC.md` and reconciled into `docs/ARCHITECTURE.md` as amendments A1–A7. Two consequences reach past presentation and are worth reading at requirement level: hue is domain identity and **never** encodes score, with score drawn as a water line (this is what makes F21's eight-region identity survive a map full of low scores); and the map gains a two-level stepped camera, so `/d/<domainId>` becomes a camera state rather than a listing — the shape raised as **D28** and priced in ARCH §19.4, now adopted. **D25 is partially addressed**: the Curious Browser reaches the map on a phone and gets a first-load moment, but how they reach a compelling *tree* without starting one is untouched and remains open.
>
...
**D1–D8, D10–D11 and D13–D18 were closed by the architecture work and are recorded in `docs/ARCHITECTURE.md`.** They are marked resolved in place, with a one-line outcome and a section pointer, rather than deleted — the questions are part of the record and the answers are cited elsewhere. **D20, D23, and D26 also close in v1.4** (estimator rule, domain reassignment scope, content licence). **D19 closes in v1.5** (the visual direction, in `docs/UI-SPEC.md`), and **D25 partially**. **D12, D21, D22, D24, D25's remainder and D27 remain open** and are the live list: D12 is content work rather than a technical choice, and the remainder are product and taxonomy decisions the architecture deliberately left alone.
...
**Product and taxonomy** — *D20 and D26 resolved in v1.4, D19 in v1.5; D23 decided out of v1; D21, D22, D24, D25 (partly), D27 remain open.*
- **D19.** ~~Visual identity per domain — palette and silhouette that make each region legible as its own place.~~ **Resolved:** the visual direction is **Survey / Ordnance** — ink-on-paper cartography — specified in full in `docs/UI-SPEC.md` §4. It was chosen over three alternatives on one hard constraint: it is the only direction that carries **eight saturated hues as a single coherent document**, which is what F21's per-region identity requires, and it survives both light and dark as a token swap rather than as a second design. Two rules do the load-bearing work. **Hue is identity and never encodes score** (§4.3): each domain's plate renders at full strength at every score, and score is a ruled **water line** at height `1 − fill`. The obvious alternative — colour at `fill` opacity — is wrong and was wrong in the design's own first draft, because most domains are low-scoring most of the time and an opacity ramp therefore drains the map of exactly the identity D19 exists to create. **Silhouette is unchanged**: regions remain the eight unioned paths of ARCH D-08. Palettes are content, one `{ base, accent }` pair per domain per theme in `domains.yaml`. `docs/UI-SPEC.md` §4; ARCH §5.9, §10.5, amendments A3 and A7.
...
- **D25.** **Partially addressed** (v1.5, `docs/UI-SPEC.md`). Onboarding for the Curious Browser persona — how someone with no intention of tracking anything reaches a compelling view.

  **What is now answered:** they reach a compelling view *of the map*, on the device they are actually holding. UI-SPEC §8.1 moves the phone list substitution from a viewport size to camera level 1, so a phone visitor now sees the world map rather than a list — the previous threshold denied the map to precisely the persona the map exists to win. §5.7's first-load reveal gives them a moment on arrival, and it is designed against the constraint that on a true first load every domain score is zero, so it must be beautiful with no data in it.

  **The remainder, stated so it is not mistaken for resolved:** how that visitor reaches a compelling view of a *tree* without starting one. The map sells the territory; nothing yet sells a single skill's ladder to someone who will not tap "start". This is UI-SPEC §12 **Q4** and it has **no owner task** in phase 2.
